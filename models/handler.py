...
def validate(model, dataloader, device, normalize_method, statistic,
    ...
    print(f'RAW :  CEEloss {score[2]:7.9f}.')
    if result_file:
...
def train(train_data, valid_data, args, result_file):
    ...
    node_cnt = train_data.shape[1]-1 #训练数据的列数
    ...
    if args.optimizer == 'RMSProp':
        my_optim = torch.optim.RMSprop(params=model.parameters(), lr=args.lr, eps=1e-08)#RMSprop优化算法
    else:
        my_optim = torch.optim.Adam(params=model.parameters(), lr=args.lr, betas=(0.9, 0.999))
    my_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=my_optim, gamma=args.decay_rate)#学习率衰减
    ...
    forecast_loss = nn.CrossEntropyLoss().to(args.device)#损失函数
    ...
    for epoch in range(args.epoch):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            # print(forecast)
            # print("-" * 99)
            loss = forecast_loss(forecast.view(forecast.size(0)*forecast.size(1),forecast.size(2)),target.view(-1).long())
            cnt += 1
            loss.backward()
            my_optim.step()
        ...
        performance_metrics = dict(total_loss=loss_total / cnt)
...
def test(test_data, args, result_train_file, result_test_file):
    ...
    print('Performance on test set: A: {:5.2f} | MAE: {:5.2f} | RMSE: {:5.4f}'.format(mape, mae, rmse))